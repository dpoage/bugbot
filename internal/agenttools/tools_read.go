...
func NewReadFile(dir string) (agent.Tool, error) {
...
func NewReadFileWithCaps(dir string, caps ReadCaps) (agent.Tool, error) {
...
func NewReadFileWithDepRoots(dir string, caps ReadCaps, extraRoots *DepSourceRoots) (agent.Tool, error) {
	root, err := NewFSRoot(dir)
	if err != nil {
		return nil, err
	}
...
func (t *readFileTool) Def() llmkit.ToolDef {
	desc := fmt.Sprintf(
...
func (t *readFileTool) Run(ctx context.Context, raw json.RawMessage) (string, error) {
	...
	if err := agent.UnmarshalArgs(raw, &args); err != nil {
		return "", err
	}
...
// traversal-protected.
func NewListDir(dir string) (agent.Tool, error) {
	root, err := NewFSRoot(dir)
	if err != nil {
		return nil, err
	}
...
func (t *listDirTool) Def() llmkit.ToolDef {
	return llmkit.ToolDef{
		Name: "list_dir",
