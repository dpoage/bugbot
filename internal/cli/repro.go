...
the command exits with a graceful message rather than an error.`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// nil progress sink: the one-shot `bugbot repro` prints its
			// summary to stdout and does not own a status.json snapshot (the
			// daemon/scan do). Writing one here would race a running
			// daemon's single-writer snapshot.
