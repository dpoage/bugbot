...
//	commit B: adds new.go (3 lines) AND appends 2 new lines to old.go (5 lines)
//
// Then asserts:
//   - anchorAbsentAtRef(new.go, 1, A) == true  (file absent at A → INTRODUCED)
//   - anchorAbsentAtRef(old.go, 1, A) == false (line existed at A → PRE-EXISTING)
//   - anchorAbsentAtRef(old.go, 5, A) == true  (line beyond A's EOF → INTRODUCED)
//
// These three cases are the load-bearing matrix for the regress attribution:
// absent-file / pre-existing-line / appended-line.
func TestAnchorAbsentAtRef(t *testing.T) {
	r := newRegressTestRepo(t)

	// Commit A: old.go, 3 lines.
	r.write("old.go", "line one\nline two\nline three\n")
	a := r.commit("A: introduce old.go (3 lines)")

	// Commit B: add new.go AND extend old.go with two more lines.
	r.write("old.go", "line one\nline two\nline three\nline four\nline five\n")
	r.write("new.go", "alpha\nbeta\ngamma\n")
	b := r.commit("B: extend old.go + add new.go")

	// Sanity: A and B must be different commits — otherwise the fixture is
	// malformed and the assertions below lose their meaning.
	if a == b {
	...
	if !anchorAbsentAtRef(context.Background(), repo, "new.go", 1, a) {
		t.Errorf("anchorAbsentAtRef(new.go, 1, A) = false; want true (file added in B, absent at A => INTRODUCED)")
	}
	if anchorAbsentAtRef(context.Background(), repo, "old.go", 1, a) {
		t.Errorf("anchorAbsentAtRef(old.go, 1, A) = true; want false (line 1 existed at A => PRE-EXISTING)")
	}
	if anchorAbsentAtRef(context.Background(), repo, "old.go", 3, a) {
		t.Errorf("anchorAbsentAtRef(old.go, 3, A) = true; want false (line 3 existed at A => PRE-EXISTING)")
	}
	if !anchorAbsentAtRef(context.Background(), repo, "old.go", 4, a) {
		t.Errorf("anchorAbsentAtRef(old.go, 4, A) = false; want true (line 4 only exists after B => INTRODUCED)")
	}
	if !anchorAbsentAtRef(context.Background(), repo, "old.go", 5, a) {
		t.Errorf("anchorAbsentAtRef(old.go, 5, A) = false; want true (line 5 only exists after B => INTRODUCED)")
	}

	// At B (HEAD), all anchors must be present (sanity for the helper).
	if anchorAbsentAtRef(context.Background(), repo, "old.go", 5, b) {
		t.Errorf("anchorAbsentAtRef(old.go, 5, B) = true; want false (line 5 present at B)")
	}
	if anchorAbsentAtRef(context.Background(), repo, "new.go", 3, b) {
		t.Errorf("anchorAbsentAtRef(new.go, 3, B) = true; want false (line 3 present at B)")
	}
}

// TestAnchorAbsentAtRef_DefensiveGuards covers the conservative-empty-input
// behavior: a nil repo, an empty file, or an empty ref all produce true so
// the caller's labelling falls on the INTRODUCED side. We prefer a false
// positive INTRODUCED over a false positive PRE-EXISTING — putting the
// finding in the investigate-first bucket is the safer default.
func TestAnchorAbsentAtRef_DefensiveGuards(t *testing.T) {
	...
	if !anchorAbsentAtRef(context.Background(), nil, "a.go", 1, "HEAD") {
		t.Error("nil repo should be treated as absent-at-ref (true)")
	}
	if !anchorAbsentAtRef(context.Background(), repo, "", 1, "HEAD") {
		t.Error("empty file should be treated as absent-at-ref (true)")
	}
	if !anchorAbsentAtRef(context.Background(), repo, "a.go", 1, "") {
		t.Error("empty ref should be treated as absent-at-ref (true)")
	}
	if !anchorAbsentAtRef(context.Background(), repo, "a.go", 0, "HEAD") {
