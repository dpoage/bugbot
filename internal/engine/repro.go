...
func (d *Dispatcher) Repro(ctx context.Context, opts ReproOpts) (*ReproResult, error) {
	...
	// nil progress sink here mirrors the pre-refactor one-shot command: it
	// prints its summary to stdout and does not own a status.json snapshot
	// (the daemon/scan do). Writing one here would race a running daemon's
	// single-writer snapshot.
	rd, err := BuildReproducer(ctx, &cfg, st, opts.Target, runtime, nil)
	if err != nil {
		return nil, err
	}
